...
if (!jsonPath) {
  console.error("Erro: Especifique o caminho do arquivo JSON contendo as atualizações de placar.");
  console.log("Uso: node scripts/update-db-scores.js <caminho_do_arquivo.json>");
  process.exit(1);
}

const rawJson = fs.readFileSync(path.resolve(jsonPath), 'utf8');
...
async function main() {
  try {
    ...
    const updatedMatches = dbMatches.map(dbMatch => {
      const update = updates.find(r => r.id === dbMatch.id);
    ...
    if (updatedCount === 0) {
      console.log("[CLI Sync] Nenhuma nova partida atualizada.");
      return;
    }

    // 3. Propagate bracket dependencies
    const finalKnockoutBrackets = triggerKnockoutPropagation(updatedMatches);

    // 4. Batch upsert the changes (deduplicated by ID)
    ...
    console.log(`[CLI Sync] Sincronização concluída com sucesso! ${updatedCount} partidas atualizadas no Supabase.`);
...
// Logic helper to compute brackets to keep DB synchronized
